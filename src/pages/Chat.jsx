...
const Chat = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        setLoading(true);
        const conversationDetails = await messageService.getConversationById(conversationId);
    ...
    const handleMessageStatus = (data) => {
      if (data.conversationId === conversationId) {
        ...
        setMessages(prev => 
          prev.map(msg => ({
            ...msg,
            readAt: msg.readAt || (msg.senderId !== user.id ? data.readAt : null)
          }))
        );
      }
    };
        ...
        <div className="w-2/3 flex flex-col">
          {conversationId ? (
            ...
            <div className="flex items-center justify-center h-full">
              <p className="text-base-content/70">Select a conversation to start chatting</p>
            </div>
          )}
        </div>
      </div>
