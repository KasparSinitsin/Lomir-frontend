...
const TagSelector = ({
  ...
  const filteredSupercategories = supercategories.filter(
    (supercat) =>
...
return (
  <div className="space-y-4">
    ...
    {/* MAIN TAG DISPLAY - FIX ALL KEYS */}
    {filteredSupercategories.map((supercat, superIndex) => (
      <div key={`supercat-${supercat.id || supercat.name || superIndex}`} className="border rounded-lg">
        <div
        ...
        {expandedSupercategories[supercat.name] && (
          <div className="p-3">
            {(supercat.categories || []).map((category, catIndex) => (
              <div key={`cat-${category.id || category.name || catIndex}`} className="mb-3">
                <div
                  ...
                  <div className="ml-4 space-y-2">
                    {(category.tags || [])
                      .filter(tag => {
                        ...
                        if (!tag || tag.id == null || tag.id === '') {
                          console.warn(`[TagSelector] Missing tag ID in category "${category.name}":`, tag);
                          return false;
                        }
                        const id = parseInt(tag.id, 10);
                        const isValid = !isNaN(id) && isFinite(id);
                        if (!isValid) {
                      ...
                      .map((tag, tagIndex) => {
                        ...
                        // Compound key with multiple fallbacks
                        const uniqueKey = `tag-${tagId}-${tag.name.replace(/\s+/g, '-')}-${tagIndex}`;
    ...
    {/* ADD TAG MODAL - FIX SELECT OPTION KEYS */}
    {showAddTagModal && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-6 rounded-lg w-96 shadow-lg">
          ...
          >
            ...
            {supercategories.map((s, idx) => (
              <option 
                key={`modal-super-${s.id || s.name || idx}`} 
                value={s.id}
              >
                {s.name}
              </option>
          ...
          >
            ...
            {(supercategories.find((s) => s.id === selectedSupercategoryId)?.categories || []).map((c, idx) => (
              <option 
                key={`modal-cat-${c.id || c.name || idx}`} 
                value={c.id}
              >
                {c.name}
              </option>
